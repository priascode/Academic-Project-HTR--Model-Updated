...
def conv_block(inputs, filters, kernel_size=(3, 3), activation="relu", padding="same", name=None):
    x = keras.layers.Conv2D(filters=filters, kernel_size=kernel_size, activation=activation,
                            kernel_initializer="he_normal", padding=padding, name=name)(inputs)
    return x
...
def build_model():
  ...
  # First conv block
  x = conv_block(input_img, filters=32, name="Conv1")
  x = conv_block(x, filters=32, name="Conv2")
  x = conv_block(x, filters=32, name="Conv3")
  x = keras.layers.MaxPooling2D((2,2), name="pool1")(x)

  # Second conv block
  x = conv_block(x, filters=64, name="Conv4")
  x = conv_block(x, filters=64, name="Conv5")
  x = conv_block(x, filters=64, name="Conv6")
  x = keras.layers.MaxPooling2D((2,2), name="pool2")(x)

  # Third conv block

  x = conv_block(x, filters=128, name="Conv7")

  # Adding max pooling layer
  x = keras.layers.MaxPooling2D((1, 1), name="pool3")(x)
    
  # Reshape
  new_shape = ((image_width // 4), (image_height // 4) * 128)  # Adjusted for three more pooling layers
  ...
  output = keras.layers.Dense(num_classes, activation="softmax", name="dense2")(x)
...
# Loading the model with 7 convolutiinal layers
#  performing inference  for model which has loss:- 0.7058  and CER :- 22.0124%

custom_objects = {"CTCLayer": CTCLayer}

reconstructed_model = keras.models.load_model("./trained_models/model_c7i_100_epoch.h5", custom_objects=custom_objects)
